...
public class EndStatsUI {
    ...
    public EndStatsUI(GlideGamemode main){
        this.main = main;
    }
    ...
    public void openEndStatsGUI(Player player){
        ...
        Inventory endStatsGUI = main.server.createInventory(player, slotCount, GlideConstants.endResultTitle);
    ...
    public ItemStack generateItemFromPlayer(Player currentPlayer){
        ...
        if(main.playerUUID_toDidPlayLastRound.get(currentPlayerUUID)) {
            ...
            if (timeCompleted != 0L) {
                ...
                long AVERAGE_TIME = GlideConstants.mapIdToAverageTimeToCompleteMS[main.lastMapId];
                rank = CalculateRank.getRank(damage, timeCompleted, AVERAGE_TIME);
            }

            lore.add(ChatColor.translateAlternateColorCodes('&', "&fRank: &l" + GlideConstants.rankToColorCode.get(rank) + rank));
            if(wasBestTime){
