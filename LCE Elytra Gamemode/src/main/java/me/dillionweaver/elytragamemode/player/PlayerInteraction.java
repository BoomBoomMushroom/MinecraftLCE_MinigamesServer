...
public class PlayerInteraction {
    ...
    public PlayerInteraction(GlideGamemode main){
        this.main = main;
    }
    ...
    public void setReady(Player player, boolean isReady){
        ...
        player.getInventory().setItem(GlideConstants.setReadyItemSlot, itemStack);
    ...
    public void updateAllVotingGUIs(){
        for(Player player : main.playersInWorld){
            if(player.getOpenInventory().getTitle() == GlideConstants.votingMenuTitle){
    ...
    public void openVotingGUI(Player player){
        ...
        Inventory votingGUI = main.server.createInventory(player, slotCount, GlideConstants.votingMenuTitle);
        for(int i=0;i<slotCount;i++){
            ...
            String[] idToName = GlideConstants.idToName;

            if(availableMapIndex >= GlideConstants.votableMapIds.length){
                noItem = true;
            }
            else {
                if (row == 0) {
                    ...
                    if (col != 0 && col != 8) {
                        int currentMapId = GlideConstants.votableMapIds[availableMapIndex];
